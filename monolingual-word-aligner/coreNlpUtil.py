...
os.sys.path.insert(1, './dist-packages')
...
class StanfordNLP:
    def __init__(self):
        ...
        #self.server = ServerProxy(JsonRpc20(), TransportTcpIp(addr=("127.0.0.1", 17176)))
    def parse(self, text):
...
# 
#                              for k in xrange(1, 3):
#                                  if -1 != parseResult['sentences'][i]['dependencies'][j][k].rfind('-'):
#                                      raise ValueError
#             
#         except KeyError:
#             pass
#         except ValueError:
#             break
#     u'dependencies': [[u'', u'VP ', u'VBN parked'],
#                                    [u'', u'PP ', u'IN on']
    
    pp = PrettyPrinter()
    # print sentences, len(parseResult['sentences'])
    # pp.pprint(parseResult)
    if len(parseResult['sentences']) == 1:
        return parseResult

    # print parseResult
    # print parseResult['sentences']
